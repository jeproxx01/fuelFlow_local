...
export function Topbar({ toggleSidebar, isSidebarOpen }) {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        const response = await fetch("/api/gas-station-staff/me", {
          method: "GET",
          credentials: "include",
          headers: {
        ...
        if (!response.ok) {
          if (response.status === 401) {
            router.push("/gas-station-staff/login");
            return;
          }
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-gray-500 text-sm">gas-station-staff</span>
              </div>
            </button>
            ...
            {showDropdown && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
                <Link
                  href="/gas-station-staff/account"
                ...
                <Link
                  href="/gas-station-staff/settings"
                  className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                  onClick={() => setShowDropdown(false)}
                >
